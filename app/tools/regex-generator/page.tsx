...
import { Code, ArrowRight, ArrowDown, Copy, CheckCircle2, Loader2, Sparkles } from 'lucide-react';
...
export default function RegexGeneratorPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/regex-generator', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-background relative overflow-hidden pt-8 pb-24 px-6 md:px-12">
      ...
      <div className="absolute top-40 right-10 w-24 h-24 bg-orange-400 rounded-full border-4 border-foreground -z-10 animate-pulse" />
      <div className="absolute bottom-20 left-20 w-32 h-32 bg-cyan-400 border-4 border-foreground rotate-12 -z-10" />
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="inline-flex bg-cyan-400 text-black px-6 py-2 rounded-full border-4 border-foreground shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.9)] font-bold uppercase tracking-widest items-center gap-2 mb-8">
            <Sparkles size={20} />
            <span>AI Powered</span>
          </div>
          
          <h1 className="text-5xl md:text-8xl font-black uppercase tracking-widest text-foreground drop-shadow-[6px_6px_0px_rgba(251,146,60,1)] mb-6">
            Regex Generator
          </h1>
          <p className="text-xl md:text-2xl font-bold text-foreground/80 max-w-3xl mx-auto" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            Generate regex from plain English, or paste a regex to see what it does.
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="flex-1 flex flex-col bg-white border-4 border-foreground rounded-3xl p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.9)]">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-black uppercase tracking-wider text-black">Input (English or Regex)</h2>
              <span className="text-sm font-bold text-black/50 uppercase tracking-widest">{inputText.length} chars</span>
            </div>
            <textarea
              value={inputText}
              onChange={(e) => setInputText(e.target.value)}
              placeholder='e.g. "Match a valid student email ending in .edu" OR "^[a-z0-9]+@[a-z0-9]+\.edu$"'
              className="w-full flex-1 min-h-[300px] p-4 bg-gray-50 border-4 border-foreground rounded-xl text-lg text-black font-medium resize-none focus:outline-none focus:ring-4 focus:ring-cyan-400/50"
          ...
          <div className="flex items-center justify-center py-4 lg:py-0">
            <motion.button
              ...
              className="bg-orange-400 text-black p-6 rounded-full border-4 border-foreground shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:shadow-[6px_6px_0px_0px_rgba(255,255,255,0.9)] flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed transition-all z-10"
            >
              {isLoading ? (
                <Loader2 size={32} strokeWidth={3} className="animate-spin" />
              ) : (
                <>
                  <Code size={32} strokeWidth={3} className="hidden lg:block" />
          ...
          {/* Output Section */}
          <div className="flex-1 flex flex-col bg-cyan-300 border-4 border-foreground rounded-3xl p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(255,255,255,0.9)] relative">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-black uppercase tracking-wider text-black">Output</h2>
              <button 
                onClick={handleCopy}
                disabled={!outputText}
