# 🎒 tollypips

**Your all-in-one backpack of tools. Fast, free, and unapologetically bold.**

tollypips is a collection of utilities designed specifically to turbocharge workflows for professionals and students alike. Featuring a highly interactive, Neo-Brutalist design.

---

## ✨ Features

- **⚡ Fast & Free:** Access premium-level tools without spending a dime.
- **🔓 No Login Required:** Jump straight into the action. No tedious sign-ups or email verifications needed.
- **🎨 Unapologetically Bold:** A stunning Neo-Brutalist design language built with Framer Motion and Tailwind CSS v4.
- **🛠️ The Arsenal:** A growing collection of highly useful tools and browser extensions.

## 🧰 The Toolkit

1. **[Context Passport](https://github.com/iamshubham1704/passport.git)**
   - An intelligent browser extension to save, compress, and transfer your AI chat context across different platforms (ChatGPT, Claude, Gemini, etc).

2. **PDF Editor**
   - A fully-featured, lightning-fast PDF editor built directly into your browser. No server uploads required—your documents never leave your device.
   - **Features include:** Add and edit text with custom Google Fonts (Roboto, Montserrat, Poppins), insert and resize images, redact content with a white-out tool, full Undo/Redo history tracking, and mobile touch support including pinch-to-zoom scaling.

3. **Tolly Humaniser**
   - Converts your AI-generated articles into natural, conversational human language.

4. **[Mail-N-End](https://mainn-end.vercel.app/)**
   - A cold email campaign manager with dynamic variables, personalized scheduling, and deliverability performance tracking.

5. **Time & Attendance Manager** _(Coming Soon)_
   - Track your work or study hours with ease. Get smart insights and manage your schedule effortlessly.

## 🚀 Tech Stack

- **Framework:** Next.js (App Router)
- **Language:** TypeScript
- **Styling:** Tailwind CSS v4
- **Animations:** Framer Motion
- **Icons:** Lucide React
- **PDF Engine:** pdf-lib, react-pdf, and @pdf-lib/fontkit
- **Analytics:** Vercel Analytics

## 💻 Running Locally

1. **Clone the repository:**

   ```bash
   git clone https://github.com/iamshubham1704/tollypips.git
   cd tollypips
   ```

2. **Install dependencies:**

   ```bash
   npm install
   ```

3. **Start the development server:**

   ```bash
   npm run dev
   ```

4. **Explore the app:** Open `http://localhost:3000` in your browser.

## 🤝 Contributing

Contributions are always welcome! If you have an idea for a new tool that professionals or students would love, feel free to submit a Pull Request or open an Issue.

---

_Made with ❤️_
